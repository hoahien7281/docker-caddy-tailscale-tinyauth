...
function run(cmd,argv,{input}={}){const shown=/(chpasswd)/.test(cmd)?`${cmd} <hidden>`:`${cmd} ${argv.join(" ")}`;log(`${dry?"[DRY RUN] ":""}${shown}`);if(dry)return{ok:true,out:"",err:""};const r=spawnSync(cmd,argv,{encoding:"utf8",input,timeout:120000});return{ok:r.status===0,out:(r.stdout||"").trim(),err:(r.stderr||r.error?.message||"").trim()}}
...
function create(u){
 ...
 if(!exists(u.user)){const a=["-m","-s",u.shell];if(u.uid)a.push("-u",String(u.uid));a.push(u.user);const r=privileged("useradd",a);if(!r.ok)throw new Error(r.err)}
